...
//An input string is being passed in and the problem says it always starts with a number. Function is also supposed to return an integer which means I have to check how to return integers from strings.
//Solution can either be gotten by using the parseInt() method on the parameter the function is taking in which is inputString or making sure the second index of the the string is strictly numerical using the Number() method.

//I prefer the parseInt() method so solution goes thus
...
// This is pretty straightforward. A variable needs to be declared to store the sum of the scores and take an average. My spaghetti code solution below
...
//Another solution I discovered and really liked is
function getGrade (s1, s2, s3) {
  var avg = (s1 + s2 + s3)/3;
  switch(true) {
    case (avg >= 90):
...
function cockroachSpeed(s) {
  //Good Luck!
  //speed will always be distance over time
  //convert distance from km to cm
  //convert time from hour to second
  //round down the result
  let distanceConversion = 100000;
  let timeConversion = 3600;
